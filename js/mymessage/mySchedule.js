	...
	for(var i=0;i<meetingJsonArray.length;i++){
		...
		if(meeting != undefined){
			var inShow = false;
				if(name!=""){
					// alert(time)
					if(meeting.title.indexOf(name)!=-1&&meeting.eid==loginUserId){
				...
				if(inShow){
					...
					var trStr = "<tr>"+
						...
						"<td><img onclick='changeMeeting("+meeting.id+")' src='../img/修改信息.png' /></td>"+
						"</tr>";
...
//无论是什么数据类型，都用var声明。而在括号的形参，则不需写var
function changeMeeting(id){
	//跳转到修改部门页面时，想要传递一个标识，能够区分想要修改的指定部门
	//传入指定要修改的部门的部门编号
	//传递部门编号
	window.location.href = "changeMeeting.html?id="+id;
}